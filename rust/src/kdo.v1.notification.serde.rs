...
impl serde::Serialize for FutureLpPageTarget {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
...
impl<'de> serde::Deserialize<'de> for FutureLpPageTarget {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = FutureLpPageTarget;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct kdo.v1.notification.FutureLpPageTarget")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<FutureLpPageTarget, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
        ...
        deserializer.deserialize_struct("kdo.v1.notification.FutureLpPageTarget", FIELDS, GeneratedVisitor)
    }
}
