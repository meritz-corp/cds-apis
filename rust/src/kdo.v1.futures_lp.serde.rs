...
impl serde::Serialize for FuturesLpHedge {
    #[allow(deprecated)]
    fn serialize<S>(&self, serializer: S) -> std::result::Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
        ...
        let mut struct_ser = serializer.serialize_struct("kdo.v1.futures_lp.FuturesLpHedge", len)?;
        if true {
...
impl<'de> serde::Deserialize<'de> for FuturesLpHedge {
    #[allow(deprecated)]
    fn deserialize<D>(deserializer: D) -> std::result::Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        ...
        struct GeneratedVisitor;
        impl<'de> serde::de::Visitor<'de> for GeneratedVisitor {
            type Value = FuturesLpHedge;

            fn expecting(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                formatter.write_str("struct kdo.v1.futures_lp.FuturesLpHedge")
            }

            fn visit_map<V>(self, mut map_: V) -> std::result::Result<FuturesLpHedge, V::Error>
                where
                    V: serde::de::MapAccess<'de>,
            {
        ...
        deserializer.deserialize_struct("kdo.v1.futures_lp.FuturesLpHedge", FIELDS, GeneratedVisitor)
    }
}
